...
YEAR_OF_LAST_REV=$(fmt -200 "$CODE.txt" |
              ...
              sed -e 's/[.,]//g' |
              tail -1)
which units &> /dev/null && echo "Course age: $(units $(date +%s)sec-$(date -d "$YEAR" +%s)sec 'yr;mo;d' -o%.2f)"
which units &> /dev/null && echo "Course age since 'valid from date': $(units $(date +%s)sec-$(date -d "$YEAR_OF_LAST_REV" +%s)sec 'yr;mo;d' -o%.2f)"
echo "Lärandemål"
cat "$CODE.txt" |
    grep .|
